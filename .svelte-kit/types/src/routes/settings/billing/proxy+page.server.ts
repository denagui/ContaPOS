...
export const load = async ({ locals, db }: Parameters<PageServerLoad>[0]) => {
...
export const actions = {
  ...
  save: async ({ request, locals, db }: import('./$types').RequestEvent) => {
    if (!locals.user) {
  ...
  testConnection: async ({ request, locals, db }: import('./$types').RequestEvent) => {
    if (!locals.user) {
      return fail(401, { error: 'No autorizado' });
    }
  ...
  delete: async ({ locals, db }: import('./$types').RequestEvent) => {
    if (!locals.user) {
      return fail(401, { error: 'No autorizado' });
    }
